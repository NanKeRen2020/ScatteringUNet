...
# from istc_l1 import ISTC



class ScatteringTorch2D_wph(ScatteringTorch, ScatteringBase2D):
    def __init__(self, J, shape, L=8, A=4, max_order=2, pre_pad=False, backend='torch', sample=True):
        ScatteringTorch.__init__(self)
        scat_args = locals()
        scat_args.pop('A')
        scat_args.pop('sample')
        ...
        # ScatteringBase2D.create_filters(self, sample)
        ScatteringBase2D.create_filters(self)

        # self.pad_size0 = self.pad_size
    ...
    def scattering(self, input):
        ...
        if self.sample:
            ...
            S = phase_scattering2d_unsample(input, self.pad, self.unpad, self.backend, self.J, self.L, self.A, phi, psi,
                               self.max_order, self.phases)
...
def phase_scattering2d(x, pad, unpad, backend, J, L, A, phi, psi, max_order, phases):
    ...
    # print(isinstance(fft, types.FunctionType))  # 输出 True
    # print(type(fft) is types.FunctionType)       # 输出 True
    # print(isinstance(fft, types.ModuleType))  # 输出 True
    # print(type(fft) is types.ModuleType)       # 输出 True
    # print(inspect.isfunction(fft))   # 输出 True
    # print(inspect.isbuiltin(fft))          # 内置函数输出 True

    # # 检查模块
    # print(inspect.ismodule(fft))            # 输出 True
...
def phase_scattering2d_scale(x, pad, unpad, backend, J, L, A, phi, psi, max_order, phases):
    subsample_fourier = backend.subsample_fourier
    modulus = backend.modulus
    fft = backend.fft
    cdgmm = backend.cdgmm
    ...
    for n1 in range(len(psi)):
        ...
        if A == 4:
            ...
            for i in range(4):
                out_S_1.append(S_1_r[:, i, :, :])
                out_S_1_scale[j1].append(S_1_r[:, i, :, :])
    ...
    out_S_scale = [[] for _ in range(J)]
    out_S_scale[0].append(out_S_0)
    for s in range(J):
        out_S_scale[s].append(out_S_1_scale[s])
        out_S_scale[s].append(out_S_2_scale[s])
        out_S_scale[s] = concatenate(out_S_scale[s])

    
    return out_S


def phase_scattering2d_multiscale(x, pad, unpad, backend, J, L, A, phi, psi, max_order, phases):
    subsample_fourier = backend.subsample_fourier
    modulus = backend.modulus
    fft = backend.fft
    cdgmm = backend.cdgmm
    ...
    multifeatures = { }
    for s in range(J):
        out_S_0_, out_S_1_, out_S_2_ = [], [], []
        multifeatures[s] = [out_S_0_, out_S_1_, out_S_2_]
        U_1_c = subsample_fourier(U_1_c_backup, k=2 ** s)
        S_0 = fft(U_1_c, 'C2R', inverse=True)
        S_0 = unpad(S_0)
        multifeatures[s][0].append(S_0)
    ...
    for n1 in range(len(psi)):
        ...
        if A == 4:
            ...
            # other scale 
            for s in range(J - j1):
                S_1_c = subsample_fourier(S_1_c_backup, k=2 ** s)
                S_1_r = fft(S_1_c, 'C2R', inverse=True)
                S_1_r = unpad(S_1_r)
                S_1_r = S_1_r.reshape(-1, 4, S_1_r.shape[-2], S_1_r.shape[-1])
                for i in range(4):
                    multifeatures[s][1].append(S_1_r[:, i, :, :])            
            ...
            for phase in phases:
                ...
                for s in range(J - j1):
                    ...
                    S_1_r = fft(S_1_c, 'C2R', inverse=True)
                    S_1_r = unpad(S_1_r)
                    multifeatures[s][1].append(S_1_r)      
        ...
        for n2 in range(len(psi)):
            ...
            # other scale 
            for s in range(J - j2):
                S_2_c = subsample_fourier(S_2_c_backup, k=2 ** s)
                S_2_r = fft(S_2_c, 'C2R', inverse=True)
                S_2_r = unpad(S_2_r)
                multifeatures[s][2].append(S_2_r)    
    ...
    for s in range(J):
        out_S = []
        out_S.extend(multifeatures[s][0])
        out_S.extend(multifeatures[s][1])
        out_S.extend(multifeatures[s][2])
        multifeatures[s] = concatenate(out_S)
    
    return out_S


def phase_scattering2d_unsample(x, pad, unpad, backend, J, L, A, phi, psi, max_order, phases):
    subsample_fourier = backend.subsample_fourier
    modulus = backend.modulus
    fft = backend.fft
    cdgmm = backend.cdgmm
    ...
    S_0 = fft(U_1_c, 'C2R', inverse=True)
    S_0 = S_0[..., unpad_size:0-unpad_size, unpad_size:0-unpad_size]
    # S_0 = unpad(S_0)
    ...
    for n1 in range(len(psi)):
        ...
        if A == 4:
            ...
            S_1_r = fft(S_1_c, 'C2R', inverse=True)
            # S_1_r = unpad(S_1_r)
            S_1_r = S_1_r[..., unpad_size:0-unpad_size, unpad_size:0-unpad_size]
            S_1_r = S_1_r.reshape(-1, 4, S_1_r.shape[-2], S_1_r.shape[-1])
            for i in range(4):
                out_S_1.append(S_1_r[:, i, :, :])
        ...
        for n2 in range(len(psi)):
            ...
            S_2_r = fft(S_2_c, 'C2R', inverse=True)
            S_2_r = S_2_r[..., unpad_size:0-unpad_size, unpad_size:0-unpad_size]
            # S_2_r = unpad(S_2_r)
