...
def noise(x, mask, noise_size = 16, target_size = [], noise_std = 0.2):

    ns = torch.normal(mean=torch.zeros(x.shape[0], x.shape[1], noise_size, noise_size), std=noise_std).to(x.device)

    ns = F.upsample_bilinear(ns, size=target_size)
...
class Train(object):
    def __init__(self, config_file: str):
        ...
        if self.config['MODEL_TYPE'] == 3:
            self.model = Model.UNetScattering(1, 11).to(self.device)   
        elif self.config['MODEL_TYPE'] == 2:
            self.model = Model.UNet(1, 11).to(self.device) 
        # elif self.config['MODEL_TYPE'] == 1:
        #     self.model = Model.ModelScattering().to(self.device)   
        else:
            self.model = Model.Model().to(self.device)
        # self.model = Model.ModelScatteringSparse().to(self.device)
        # self.model = Model.UNet(11).to(self.device)
...
epoch
    ...
    parser.add_argument('--cfg_pth', type=str,
                        ...
                        default='/home/nankeren/AI_projects/segment/m2nist-segmentation0/pytorch/config.yaml')
